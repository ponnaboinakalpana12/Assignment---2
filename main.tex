...
\item Now using \eqref{eq13} we get:
\begin{align}
&\implies \vec{S}=\myvec{a\\0} +b\myvec{\cos M \\\sin M}
\\
...
Now, using,
\begin{align}
\vec{T}=d \myvec{\cos M\\\sin M }
\end{align}
\\
\item Using \eqref{eqb}and where P =$-3.97$ solving we get:
\begin{align}
d= \frac{-3.97[0.19-0.59+0.96]-42.25+3.95}{[-3.97-0.96+0.19]}
\end{align}
\begin{align}
\implies d=8.54
\\
\implies \vec{T}=8.54 \myvec{\cos 100\degree\\\sin 100\degree}
\implies \vec{T}=\myvec{-1.48\\8.41}
\end{align}
\item Now,the vertices of given Quadrilateral MIST can be written as,
\begin{align}
 \vec{M}=\myvec{0\\0},\vec{I} = \myvec{3.5\\0}, \vec{S}=\myvec{2.37\\6.40},\vec{T}=\myvec{-1.48\\8.41}
\end{align}
    \item On constructing the quadrilateral $MIST$ we get:
\end{enumerate}
\end{itemize}
...
\begin{figure}[!ht]
\centering
\includegraphics[ width=\columnwidth, height=08 cm]{Fig2.pdf}
\caption{Quadrilateral MIST}
\label{fig:Quadrilateral MIST}	
\end{figure}
\end{document}